    ...
    addUsers.off('click').on('click', function(){
        ...
        if(!rVagonFilledBy || rVagonFilledBy == 'agents'){
            ...
            function afterDataGetted(){
                    ...
                    roleItem.off('click').on('click', function(){
                        ...
                        if(id == 'all'){
                            socketQuery({
                                ...
                                var usersByRoleNames = res.data_columns;

                                for(var i in res.data){
                                    var ubr = res.data[i];
                                    var userId = ubr[usersByRoleNames.indexOf('USER_ID')];
                                    if($.inArray(userId, flatUsersInFund) == -1){
                                        lBMObj.users.push({
                                            userId: userId,
                                            name: ubr[usersByRoleNames.indexOf('FULLNAME')]
                                        });
                                    }
                                }
                    ...
                    $(formTrain.rolesSelInstance).off('changeVal').on('changeVal', function(e, was, now){
                        ...
                        if(roleId == 'all'){
                            socketQuery({
                                ...
                                usersInFund = res;
                                for(var ui in usersInFund.data){
                                    var uiItem = usersInFund.data[ui];
                                    flatUsersInFund.push(uiItem[inFundNames.indexOf('USER_ID')]);
                                }
                            ...
                            socketQuery({
                                ...
                                for(var i in res.data){
                                    var userItem = res.data[i];
                                    var userId = userItem[usersByRole.data_columns.indexOf('USER_ID')];

                                    if($.inArray(userId, flatUsersInFund) == -1){
                                        lBMObj.users.push({
                                            userId: userId,
                                            name: userItem[usersByRole.data_columns.indexOf("FULLNAME")]
                                        });
                                    }
                                }
                        ...
                        for(var i =0; i< rb.find('.uff-item').length; i++){
                            ...
                            socketQuery({
                                ...
                                if(res['code'] == 0){
                                    ...
                                    insideItem.addClass('toRemove').hide(0);
                                }else{
                                    toastr[res['toastr']['type']](res['toastr']['message']);
                                }
                ...
                var roleNames = roles.data_columns;
                var userNames = allUsers.data_columns;
                var inFundNames = usersInFund.data_columns;
                ...
                for(var r in roles.data){
                    var roleItem = roles.data[r];
                    roleMObj.roles.push({
                        roleId: roleItem[roleNames.indexOf('ROLE_ID')],
                        roleName: roleItem[roleNames.indexOf('NAME')]
                    });
                }
    ...
    addAgents.off('click').on('click', function(){
        if(!rVagonFilledBy || rVagonFilledBy == 'users'){
            ...
            function afterDataGetted(){
                ...
                for(var ui in usersInFund.data){
                    var uiItem = usersInFund.data[ui];
                    flatUsersInFund.push(uiItem[inFundNames.indexOf('AGENT_ID')]);
                }

                for(var u in allUsers.data){
                    var userItem = allUsers.data[u];
                    var userId = userItem[userNames.indexOf('AGENT_ID')];
                    if($.inArray(userId, flatUsersInFund) == -1){
                        lBMObj.users.push({
                            userId: userId,
                            name: userItem[userNames.indexOf('NAME')]
                        });
                    }
                }
