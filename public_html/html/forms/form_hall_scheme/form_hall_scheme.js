(function () {
    ...
    instance.custom = function (callback) {
        if (instance.mode !== "add") {
            ...
            var title = instance.data.data[0][instance.data.names.indexOf('HALL_NAME')];
            var CustomButtons = {
                goToRedactor: {
                    ...
                    callback: function (key, options) {
                        MB.Core.sendQuery({command:"get",object:"hall_scheme",sid:MB.User.sid,params:{where:"hall_scheme_id = "+id}},function(data){
                            var obj = MB.Core.jsonToObj(data);
                            var hall_id = obj[0].HALL_ID;
                            MB.Core.switchModal({type:"content", filename:"mapEditor", params:{hall_scheme_id:id,hall_id:hall_id, title: title, label: 'Редактор зала'}});
                        });
                    }
                },
                goTocreatCopy: {
                    ...
                    callback: function (key, options) {
                        var hallName = instance.data.data[0][instance.data.names.indexOf("NAME")]+" (Копия)";
                        var modalObj = {
                            ...
                            buttons: {
                                ok1: {
                                    ...
                                    callback: function(){
                                        ...
                                        MB.Core.sendQuery({command:"operation",object:"copy_hall_scheme",sid:sid,params:{hall_scheme_id:id,all:0,name:copySchemeName}},function(data){
                                            info.fadeOut(600);
                                            if (data.RC==0){
                                ...
                                ok2: {
                                    ...
                                    callback: function(){
                                        ...
                                        var copySchemeName = $(".copySchemeName").val();
                                        MB.Core.sendQuery({command:"operation",object:"copy_hall_scheme",sid:sid,params:{hall_scheme_id:id,name:copySchemeName,all:1}},function(data){
                                            info.fadeOut(600);
                                            if (data.RC==0){
                                                instance.reload("data");
                                                toastr[data.TOAST_TYPE](data.MESSAGE, data.TITLE);
