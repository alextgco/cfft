...
//                console.log(primalData);
	...
	function loadCurrentPricing() {
		...
		MB.Core.sendQuery({
			...
			var obj = MB.Core.jsonToObj(data);
			console.log(obj, environment.price_zone_pricing_id);
			var html = '';
	...
	MB.Core.sendQuery({
		...
		environment.hall_scheme_id = obj[0].HALL_SCHEME_ID;
			...
			var layerO = {
				...
				sid:sid,
				params:{
					where:"ACTION_ID = "+environment.activeId+" and VISIBLE_ADMIN='TRUE'",
				...
				bootbox.dialog({
					...
					buttons: {
						success: {
							...
							callback: function () {
								MB.Core.sendQuery({
									command: "operation", object: "fill_action_scheme_by_price_group", sid: sid, params: {
										action_id: environment.activeId,
										price_group_id: "",
										all: 1
									}
