		...
		load: function () {
			if (environment.order_id == undefined) {
				...
				MB.Core.sendQuery({
					...
					for (var k in obj) {
						contentWrapper.find("." + self.box).append('<tr class="one_place" id="one_place' + obj[k]["ACTION_SCHEME_ID"] + '">' +
					...
					contentWrapper.find('.totalPlaces').html(data.TOTAL_TICKETS);
		...
		clear_blocked_place: function () {
			...
			MB.Core.sendQuery({
				...
				one_action_map.reLoad(function () {
					action_price_info.load();
					toastr[data['TOAST_TYPE']](data['MESSAGE']);
				});
				tickets_stack.load();
			});
		},
