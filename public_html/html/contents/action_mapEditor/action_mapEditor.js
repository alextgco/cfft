...
//ELSIF operation_object_ = 'generate_action_scheme_items' THEN
//ELSIF operation_object_ = 'new_action_scheme_item' THEN
//ELSIF operation_object_ = 'modify_action_scheme_item' THEN
//ELSIF operation_object_ = 'delete_action_scheme_item' THEN


function action_mapEditor_init(id){
    ...
    if(environment.title){
        Modaltitle = 'Редактор схемы мероприятия"'+environment.title+'"';
    }else{
        Modaltitle = 'Редактор схемы мероприятия';
    ...
    mapEditor_map.id = id;
    //m = mapEditor_map;
    mapEditor_map.isActionEditing = true;
    ...
    mapEditor_map.removeFileServer = function(file_ids){
        ...
        for (var k in file_ids){
            files.push({
                action_file_list_id: file_ids[k]
            });
        }
        //log(objects);
        var obj = {
    ...
    mapEditor_map.removeLayerServer = function(layer_ids,callback){
        ...
        for (var k in layer_ids){
            var obj = {
                ...
                sid:sid,
                params:{
                    action_scheme_layer_id:layer_ids[k]
                }
            };
            socketQuery(obj,callback);
        }
    ...
    mapEditor_map.addObjectServer = function(object,callback){
        ...
        var save_obj = {
            ...
            ACTION_SCHEME_LAYER_ID:(!isNaN(+object.layer_id))?object.layer_id:'',
    ...
    mapEditor_map.removeObjectServer = function(object_ids,callback){
        ...
        for (var k in object_ids){
            var obj = {
                ...
                sid:sid,
                params:{
                    action_scheme_object_id:object_ids[k]
                }
            };
            socketQuery(obj,callback);
        }
    ...
    mapEditor_map.container.on('removeAreaGroup_callback',function(e,obj){
        console.log('removeAreaGroupCallback', obj);
        if(obj && obj.ids){
            var isAreaGroupRemoveError = false;
            for(var i in obj.ids){
                if(obj.ids[i] == null){
                    isAreaGroupRemoveError = true;
                }
            }
            if(isAreaGroupRemoveError){
                toastr['error']('Нельзя удалять зоны, к которым принадлежат заблокированные, забронированные и проданные места', 'Внимание!');
    ...
    mapEditor_map.container.on('loadAreaGroup_callback',function(e,obj){
        //$('#modal_'+mapEditor_map.id+'_wrapper').trigger('zonesUpdate');
    });
    ...
    mapEditor_map.generateSquaresServer = function(params,callback){
        ...
        var obj = {
            command:"operation",
            object:"generate_action_scheme_items",
            params:{
                action_id:environment.action_id,
                ...
                ACTION_SCHEME_LAYER_ID:params.layer_id || "",
                ACTION_SCHEME_OBJECT_ID:params.object_id || 0
            },
            sid:sid
    ...
    mapEditor_map.container.on('removeSquare_callback',function(e,obj){
        var isUnavaliableDeleted = false;
        if(obj && obj.ids){
            for(var i in obj.ids){
                if(obj.ids[i] == null){
                    isUnavaliableDeleted = true;
                }
            }
            if(isUnavaliableDeleted){
                toastr['error']('Заблокированные, проданные и зарезервированные места нельзя удалять', 'Внимание!');
                mapEditor_map.reLoad();
                mapEditor_map.clearSelection();
            }
        }
...
////*****--------------------------------------------------------------------------------------------------------**//
    ...
    mapEditor_map.loadObjectServer = function(obj,callback){
        ...
        var o = {
            ...
            sid:sid,
            params:{
                action_id: environment.action_id //+" and OBJECT_TYPE = '"+type+"'"
            }
        ...
        if (obj.layer_id!=undefined)
            if(obj.layer_id != "isNULL")
                "ACTION_SCHEME_LAYER_ID="+obj.layer_id;
            else
                "ACTION_SCHEME_LAYER_ID is null";
    ...
    var socketObject = {
        ...
        load:{
            command:"get",
            object:"action_scheme_for_editor",
            params:{
                action_id:environment.action_id
            },
    ...
    layerO = {
        ...
        portion:100,
        params:{
            where:"ACTION_ID = "+environment.action_id+" and VISIBLE_EDITOR='TRUE'",
        ...
        bootbox.dialog({
            ...
            buttons: {
                ok: {
                    ...
                    className: "red",
                    callback:function(){
                        MB.Core.sendQuery({command:"operation",object:"remove_all_action_scheme_items",sid:sid,params:{action_id:environment.action_id}},function(data){
                            if (data.RC==0){
   ...
   /* mapEditor_map.container.on('add_squares',function(e,obj){
    ...
    mapEditor_map.container.on('move_squares_end',function(e){
        ...
        for (var i in mapEditor_map.selection){
            obj.push({
                action_scheme_id: mapEditor_map.squares[mapEditor_map.selection[i]].id,
    ...
    mapEditor_map.container.on('move_object_end',function(e,objects){
        ...
        for (var k in objs){
            toModify.push({
                action_scheme_object_id:objs[k].object_id,
                X:objs[k].x,
                Y:objs[k].y
    ...
    mapEditor_map.container.on('transform_object_end',function(e,object){
        ...
        mapEditor_map.container.trigger("modifyObject",[[{
            action_scheme_object_id:object.object_id,
       ...
       /* var line_start = +params.line_start || 1;
        ...
        for (var r in obj){
            ...
            for (var p in obj[r]){
                ...
                sq.push({
                    action_scheme_id:mapEditor_map.squares[obj[r][p]].id,
                    line:line,
                    place:String(place)
                });
    ...
    mapEditor_map.xScale = function(dX){
        ...
        for (var k0 in mapEditor_map.selection){
            ...
            /*sq.push({
                action_scheme_id:mapEditor_map.selection[k0],
                x:minX +delta
            });*/
        }
    ...
    var renderObjects = [];
        mapEditor_map.container.trigger('loadLayer',[{},function(result){
            ...
            for (var l in obj){
                ...
                if (obj[l].ACTION_SCHEME_LAYER_ID!="isNULL"){
                        layer = new MELayer({
                            id:obj[l].ACTION_SCHEME_LAYER_ID,
                            title:obj[l].NAME
                    });
                ...
                mapEditor_map.container.trigger('loadObject',[{layer_id:obj[l].ACTION_SCHEME_LAYER_ID},function(result2){
                    counter2++;
                    ...
                    for (var i in obj){
                        ...
                        var layer_id =  (obj[i].ACTION_SCHEME_LAYER_ID!='')?obj[i].ACTION_SCHEME_LAYER_ID:'placeLayer';
                        var layer = editor.findLayer(layer_id);
                        var params = obj[i];
                        if (params.OBJECT_TYPE=="") continue;
                        var o = {
                            object_id : params.ACTION_SCHEME_OBJECT_ID || undefined,
