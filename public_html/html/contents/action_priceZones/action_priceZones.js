...
function action_priceZones_init(id){
    ...
    function loadPriceZoneGroups(){
        ...
        MB.Core.sendQuery({command:"get",object:"price_group_for_action_scheme",sid:sid,params:{action_id:environment.action_id, show_all: isUsedOnly}},function(data){
            var obj = MB.Core.jsonToObj(data);
            var html = '';
                                    ...
                                    bootbox.dialog({
                                        ...
                                        buttons: {
                                            ok: {
                                                ...
                                                className: "red",
                                                callback: function() {
                                                    MB.Core.sendQuery({command:"operation",object:"fill_action_scheme_by_price_group",sid:sid,params:{
    ...
    function loadPricingList(){
        /// загрузка расценок
        if (typeof environment.action_id=="undefined") return;
        MB.Core.sendQuery({command:"get",object:"action_scheme_pricing",sid:sid,params:{where:"action_id = "+environment.action_id+" and DEFAULT_ACTION_PRICING_ID='TRUE'"}},function(data){
            var obj = MB.Core.jsonToObj(data);
...
*/
    ...
    *//*    action_priceZones_map.box.contextmenu(function(e){
    ...
    $('#modal_'+id+'_wrapper #toFundsZones').on('click', function(){
        MB.Core.switchModal({type:"content",filename:"action_fundZones", params:{
            action_id: environment.action_id ,
    ...
    $('#modal_'+id+'_wrapper #toPriceGroups').on('click', function(){
        MB.Core.switchModal({type:"table",name:"table_price_group",params:{callback: function(){
            loadPriceZoneGroups();
        }}});
        // waiting
    });
