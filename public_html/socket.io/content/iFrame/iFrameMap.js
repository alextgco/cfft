var doc_root = "https://shop.mirbileta.ru";
...
function MapWeb(params){
    ...
    this.loadBackgroundFlag = false;
    this.chairImg = new Image();
    this.skinSprite = new Image();
    this.skinSpriteLoaded = false;
    this.skins = [];
    ...
    this.skinSprite.onload = function(){
        self.skinSpriteLoaded = true;
        //self.render();
    ...
    this.init = function(){
        ...
        this.backgroundImage.onload = function(){
            ...
            self.loadBackgroundFlag = true;
            //this.backgroundImageLoaded = true;
        };

        this.backgroundImage.src = "img/BG.png";
        this.shadowImage.src = "img/shadow2.png";
    ...
    this.setBoxSize = function(){
        ...
        if (params!=undefined){
            ...
            $("#"+params.name).width(cWidth);
            $("#"+params.name).height(cHeight);
        ...
        /*this.box.html("<canvas height='"+cHeight+"' width='"+cWidth+"' id='canvas1'>Обновите браузер</canvas>" +
            "<div id='buy_box'>" +
            "<div class='left p100'  id='placeCount'></div>" +
            "<div class='left p100' id='amount'></div>" +
            "<div class='left' id='pay_btn'>Оплатить</div>" +
            "</div>"+
            "<div class='box_for_zoom'  style='display: block;' id='"+params.name+"_zoom'>" +
            "<canvas height='110' width='200' id='canvas2'>Обновите браузер</canvas>" +
            "</div>" +
        ...
        this.box.width(cWidth);
        this.box.height(cHeight);
        this.toPointX = 200/2;
        this.toPointY = 110/2;
    ...
    this.normalizeCanvasSize = function(){
        ...
        this.canvas2.attr("width","200px");
        this.canvas2.attr("height","110px");
...
*/
    ...
    this.createSquares = function(data){
        ...
        for (var k in DATA){
            ...
            var index =  DATA[k].ACTION_SCHEME_ID ;
            //log(+DATA[k].W);
            this.squares[index] = {};
            ...
            this.squares[index].textStatus = (DATA[k].STATUS==1)? "Свободно" : "Не доступно";
            ...
            this.squares[index].skin = (DATA[k].PRICE_GROUP_NAME!=undefined)?DATA[k].PRICE_GROUP_NAME:1;
        ...
        var self = this;
        if (this.firstLoad){
            self.chairImg.src = "img/vvv3.png";
            self.skinSprite.src = "img/chairSprite.png";
            ...
            self.box.mousewheel(function(e,delta){
                ...
                if (self.scaleCoeff>=1.4){

                    if (self.layoutHCount!=1){
                        self.layoutHCount = 1 ;
                        self.layoutVCount = 1 ;
                        self.setLayout();
                    }
                }else{
                    if (self.layoutHCount!=20){
                        self.layoutHCount = 20 ;
                        self.layoutVCount = 20 ;
                        self.setLayout();
                    }
                }
                                ...
                                if (square){
                                    ...
                                    self.showStatus({
                                      ...
                                      /*  x:x+20,
                                        ...
                                        status_line:self.squares[square].line,
                                        status_place:self.squares[square].place,
...
////for (var key in mp.squares){send_query({command:"edit",object:"block_by_cashier",sid:sid,params:{action_scheme_id:mp.squares[key].id}},function(){});}
            ...
            self.box2.on("mouseup",function(e){
            self.mouseKey = 0;
              return false;
            ...
            self.box2.on("mouseenter",function(e){
                self.box.mouseleave();
                return false;
    ...
    this.loadObjects=function(){
        ...
        var self = this;
        sendQuery(makeQuery({command:"get",object:object,params:params,frame:frame,navigator:navigator.userAgent}),function(data){
    ...
    this.setScaleCoff = function(){
        ...
        /*var dx = Math.abs(this.maxX - this.minX);
         ...
         if (dx>=dy){
         this.scaleCoeff2 = 200/(dx+3000);
         this.box2.width(200);
         this.box2.height(110);  /// Вычислено из пропорции
         }else{
         this.scaleCoeff2 = 110/(dy+300);
         this.box2.width(110);
         this.box2.height(200);  /// Вычислено из пропорции
         }
         */
    ...
    this.showStatus = function(status){
        ...
        if (status.status_area!=undefined)
            ...
            $("#status_area").text("");
        if (status.status_line!=undefined)
            $("#status_line").text(status.status_line);
        else
            $("#status_line").text("");
        if (status.status_place!=undefined)
            $("#status_place").text(status.status_place);
        else
            $("#status_place").text("");
        ...
        /*if (status.x!=undefined && status.y!=undefined){
         if (status.y>=this.box.height()+parseInt(this.box.css("top"))-$("#hint_parent").height())
         status.y = this.box.height()+parseInt(this.box.css("top"))-$("#hint_parent").height()+10;
         $("#hint_parent").stop(true,true).animate({top:status.y+"px",left:status.x+10+"px"},450);*/
         $("#hint_parent").stop(true,true).fadeIn(450);
         //}
    };
    ...
    this.drawBackground = function(){
        ...
        var x = Math.round(1*self.scaleCoeff+self.XCoeff);
        var y = Math.round(1*self.scaleCoeff+self.YCoeff);
        ...
        if (!this.moving)
            this.ctx2.drawImage(this.backgroundImage,this.XCoeff2-13,this.YCoeff2-30,this.backgroundImage.width*3*this.scaleCoeff2,this.backgroundImage.height*3*this.scaleCoeff2);
        //self.ctx.drawImage(self.backgroundImage,x,y,self.backgroundImage.width*self.scaleCoeff,self.backgroundImage.height*self.scaleCoeff);
    };
    ...
    this.drawSquaresChild = function(key,cw,ch){
        ...
        if (this.skinSpriteLoaded){
            var img = this.skinSprite;
            if (isNaN(+square.skin)) square.skin = 1;
            this.ctx.drawImage(this.shadowImage,x-6*this.scaleCoeff,y-6*this.scaleCoeff, 51.4*this.scaleCoeff,51.4*this.scaleCoeff);
            if (square.status==0){
                if (chairImg.loaded)
                    this.ctx.drawImage(chairImg,x,y,w*this.scaleCoeff,h*this.scaleCoeff);
            }else{
                if (square.selected)
                    this.ctx.drawImage(img,71*2,(+square.skin-1)*71,71,71,x,y,40*this.scaleCoeff,40*this.scaleCoeff);
                else{
                    this.ctx.drawImage(img,0,(+square.skin-1)*71,71,71,x,y,40*this.scaleCoeff,40*this.scaleCoeff);
                }
    ...
    this.load = function(params,callback){
        ...
        var self = this;
        var xml = "<query><command>get_action_scheme</command><action_id>"+this.loadParams.params.action_id+"</action_id></query>";
        sendQuery(xml,function(data){
    ...
    this.addToSelection = function(id){
        var num = $.inArray(id,this.selection);
        if (num!=-1){
            this.removeFromSelection(num);
            this.squares[id].selected = 0;
        } else{
            this.selection.push(id);
            this.squares[id].selected = 1;
    ...
    this.countSelectionAmount = function(){
        var count = 0;
        for (var k in this.selection){
            count += +this.squares[this.selection[k]].salePrice;
        }
        return count;
    };
    this.setSelectionInfo = function(){
        var count = this.countSelection();
        var amount = this.countSelectionAmount();
        if (count>0){
            $("#placeCount").html(count);
            $("#amount").html(amount);
            if ($("#buy_box:visible").length==0)
                $("#buy_box").fadeIn(350);
        }else{
            if ($("#buy_box:visible").length>0)
                $("#buy_box").fadeIn(350);
    ...
    this.sendSelection = function(){
        ...
        var xml = makeQuery({command:"create_order",object:"",params:{id:this.selection.join(','),action_id:this.loadParams.params.action_id,frame:frame,navigator:navigator.userAgent}});
        sendQuery(xml,function(data){
            ...
            self.toAcquiropay(data);
        });
    };
    this.toAcquiropay = function(result){
        var values = {};
        ...
        values['ok_url'] = doc_root+"/content/iFrame/payment_ok.php";
        values['ko_url '] = doc_root+"/content/iFrame/payment_ko.html";
    ...
    this.loadSkins = function(){
        var xml = makeQuery({command:"get",object:"",params:{where:"action_id="+this.loadParams.params.action_id}});
    };
    this.init();
