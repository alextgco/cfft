var url_xml = "http://192.168.1.101/cgi-bin/b2c?p_xml=%3Cquery%3E%3Ccommand%3Eget_placement_map%3C/command%3E%3Cevent_item_id%3E132%3C/event_item_id%3E%3C/query%3E";
...
function change_vid(obj_this){
  ...
  switch(id){
    ...
    break;
    case "2":
      $(".place").css("box-shadow","0 0 5px #000");
    break;
  }
...
function xmlParser(xml) {
  ...
  if(rc==0){
    ...
    $(xml).find("PLACEMENT_MAP").each(function() {
      ...
      tiptip_content[values['id']]['price'] = values['price']+' руб';
      tiptip_content[values['id']]['sectionname'] = values['sectionname'];

      if(values['status'] != 1){
        values['color'] = "gray";
        values['cursor'] = "default";
        tiptip_content[values['id']]['price'] = '<span style="color:#FF3333;">место занято</span>';
      }
      var cube = '<figure class="front"></figure><figure class="back"></figure><figure class="right"></figure><figure class="left"></figure><figure class="top"></figure><figure class="bottom"></figure>';
      html+= '<div id="place'+values['id']+'" class="place status'+values['status']+'" IdPlace="'+values['id']+'" sectionname="'+values['sectionname']+'" price="'+values['price']+'" col="'+values['col']+'" row="'+values['row']+'" style="cursor: '+values['cursor']+';background:'+values['color']+';top:'+values['top']+'%;left:'+values['left']+'%" unselectable="1">'+cube+'</div>';

    })
    $(".scalebox").append(html);
    /* --------------------------------------------------------- */
    ...
    $(function() {
      ...
      $('#places div.place').each(function(index, e) {
        ...
        $(this).mouseenter(function(){
          ...
          html+= '<br>стоимость: '+tiptip_content[id]['price'];
...
function formpay(result){
  ...
  */
  ...
  /* Запихнуть формирование строки в функцию */
  /*
  var cb_url = '<query><command>callback_order</command>';
  cb_url+= '<order_id>'+values['cf']+'</order_id>';
  cb_url+= '</query>';
  */
  values['cb_url'] = "http://81.200.5.250:888/cgi-bin/acqp_callback";
  values['ok_url'] = "http://192.168.1.101/content/scene/paymay_ok.html";
  values['ko_url '] = "http://192.168.1.101/content/scene/paymay_ko.html";
  ...
  */
  ...
  var html = "";
  for(key in values){
    html+= '<input type="text" name="'+key+'" value="'+values[key]+'" />';
  }
