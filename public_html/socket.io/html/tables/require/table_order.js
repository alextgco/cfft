(function () {
	...
	instance.custom = function (callback) {
		...
		instance.$container.find(".top-panel .row").append("<div class='col-md-1'><input type='checkbox' class='statusToggler' checked></div>");
		instance.$container.find(".top-panel .statusToggler").on("click", function (e) {
			...
			// for (var key in instance.profile.general.filterWhere) {
			// 	if (key === "STATUS2" && instance.profile.general.filterWhere[key].type === "notIn") {
			// 		if (checked) {
			// 			instance.profile.general.filterWhere["STATUS2"] = {
			// 				type: "notIn",
			// 				value: "('CANCELED', 'RETURNED', 'RETURNED_REALIZATION', 'DEFECTIVE')"
			// 			};
			// 			console.log(instance.profile.general.filterWhere);
			// 			// instance.reload("data");
			// 		} else {
			// 			delete instance.profile.general.filterWhere["STATUS2"];
			// 			console.log(instance.profile.general.filterWhere);
