            ...
            if (res && status && status === "success" && typeof res === "object") {
                if (res.hasOwnProperty("RC")) {
                    if (parseInt(res.RC) === 0) {
                        ...
                        toastr ? toastr.error("Ваша сессия не актульна, зайдите на сайт пожалуйста заново", "MB.Core.sendQuery") : alert("Ваша сессия не актульна, зайдите на сайт пожалуйста заново, MB.Core.sendQuery");
                        setTimeout(function () {$.removeCookie("sid");document.location.href = "login.html";}, 3000);    
                    } else {
                        callback(res);
                        toastr.error(res.MESSAGE, "MB.Core.sendQuery");
                    }    
                } else {
                    callback(res);
                }
