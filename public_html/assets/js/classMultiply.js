...
if(MB.User.sid == undefined){
    // MB.User.sid = "TQuOYLVySrrXrqtTDGmdOxyHemJChcbS";
}
...
MultiplySelectClass.prototype.getInitData = function(callback){
    ...
    var all = {
        "sid":          MB.User.sid,
        "command":      "get",
        "object":   this.subcommandAll,
        "where":        this.whereAll,
        "params":       this.paramsAll
    };
    MB.Core.sendQuery(ex, function (exResponse) {
        MB.Core.sendQuery(all, function (allResponse) {
            var response = {};
            response['exResponse'] = MB.Core.jsonToObj(exResponse);
            response['allResponse'] = MB.Core.jsonToObj(allResponse);
            callback(response);
        }); 
    }); 
}
...
MultiplySelectClass.prototype.render = function(){
    ...
    */
    ...
    SwitchButtons("dis");
    divEx.prepend("<div><center><b>"+this.titleEx+"</b></center></div>");
    divAll.prepend("<div><center><b>"+this.titleAll+"</b></center></div>");
...
MultiplySelectClass.prototype.handleEvents = function(){
    ...
    $("#MultyPlySave").click(function(){
        ...
        for (var id in objAction["remove"]){
            ...
            MB.Core.sendQuery({command:"remove",object:_this.subcommandEx,sid:MB.User.sid,params:params},function(result){
        ...
        for (var id in objAction["add"]){
            ...
            MB.Core.sendQuery({command:"new",object:_this.subcommandEx,sid:MB.User.sid,params:params},function(result){
                SaveComplite(result,objAction);
            });
            counterCallback++;
        }
