...
var modalWindow = function(){
    ...
    var mWItem = function(params){
        ...
        this.containId =        params.containId || undefined;
    };
...
//        console.log(tryLineWidth / (tryLength+1));
    ...
    mWItem.prototype.render = function(callback){
        ...
        switch(_t.startPosition){
            case 'center':
                _t.left = ($(window).width() / 2) - (_t.width / 2);
                _t.top = (isNaN(+_t.height))? 120 : ($(window).height() / 2) - (_t.height / 2);
                break;
            case 'shift':
...
//                console.log('_t.top: ', _t.top, '_t.left: ', _t.left);
        ...
        var tryTemplate = '<div class="mw-try-wrap active" data-id="mw-{wrapId}"> {title}' +
                          '<div class="mw-try-close"><i class="fa fa-times"></i></div></div>';
        ...
        if(_t.startPosition != 'shift' && +_t.height + +_t.top > $(window).outerHeight() - $('#mw-footerPanel').outerHeight()){
            _t.stick = 'top';
            _t.stickModal();
        }
    ...
    mWItem.prototype.setHandlers = function(callback){
        ...
        blocks.footerBtn.off('click').on('click',function(){
            if(_t.collapsed){
                _t.expand();
                _t.setActive();
            }else{
        ...
        blocks.footerBtn.find('.mw-try-close').off('click').on('click', function(){
            _t.wrapper.remove();
