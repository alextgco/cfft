...
//    var mapEditor_map;
    ...
    mapEditorLogicInit(mapEditor_map);
    ...
    mapEditor_map.drawSpecialObjects = function(){
        ...
        for (var k in items){
            ...
            switch (obj.type){
                case "point":
                    var x = (+obj.object.x+obj.x1*obj.object.scaleCoef)*this.scaleCoeff+this.XCoeff;
                    var y = (+obj.object.y+obj.y1*obj.object.scaleCoef)*this.scaleCoeff+this.YCoeff;
                    this.ctx.beginPath();
                ...
                case "line":
                    ...
                    var x = (+obj.object.x+obj.x1*obj.object.scaleCoef)*this.scaleCoeff+this.XCoeff;
                    var y = (+obj.object.y+obj.y1*obj.object.scaleCoef)*this.scaleCoeff+this.YCoeff;
                    var x2 = (+obj.object.x+obj.x2*obj.object.scaleCoef)*this.scaleCoeff+this.XCoeff;
                    var y2 = (+obj.object.y+obj.y2*obj.object.scaleCoef)*this.scaleCoeff+this.YCoeff;
                    this.ctx.moveTo(x,y);
    ...
    mapEditor_map.mouseOnSpecialObject = function(x,y){
        ...
        for (var k in items){
            ...
            var itemX = +item.object.x+item.x1*item.object.scaleCoef;
            var itemY = +item.object.y+item.y1*item.object.scaleCoef;
    ...
    mapEditor_map.mouseOnObject = function(x,y){
        ...
        for (var k in items){
            ...
            switch (item.type){
                case 1:
                    if (x1>=+item.x && x1<=+item.x+item.value.width*item.scaleCoef && y1>=+item.y && y1<=+item.y+item.value.height*item.scaleCoef) {
                        return item;
                    }
    ...
    mapEditor_map.removeSquaresClient = function(square_ids){
   ...
   /* mapEditor_map.container.on('add_squares',function(e,obj){
    ...
    mapEditor_map.rotateObject = function(x,y,object,callback){
        ...
        switch (object.type){
            case 1:
                if (typeof object.value!="object") return;
                var centerX = +object.x +object.value.width*(+object.scaleCoef)/2;
                var centerY = +object.y +object.value.height*(+object.scaleCoef)/2;
