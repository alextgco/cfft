...
//            });
        ...
        getActions: function(callback){
            ...
            socketQuery(o, function(res){
                if (res == 'NOT_AVALIBLE'){
                    wrapper.html('Сервис не доступен');
                    return console.log('Сервис не доступен');
                }
                res = JSON.parse(res);
        ...
        glowPriceGroup: function(id, state){
            ...
            if(!state){
                ...
                for(var i in widget.map.squares){
                    ...
                    if(sqr.priceGroup == id && sqr.status!=0){
            ...
            var pOkCallback = function(){
                ...
                if(initData['ticketpass']){
                    ...
                    for(var i = 0; i < zonesArr.length; i++){
                        ...
                        var c = countsArr[i];
                        var idx = i+1;
                        purchaseObj.params['action_scheme_ticket_zone_id_'+idx] = z;
                        purchaseObj.params['ticket_count_'+idx] = c;
                    }
