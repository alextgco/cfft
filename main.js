(function() {
  ...
  clearLSProfiles = function() {
    var ls, profiles, prs, _i, _len, _results;
    profiles = [];
    ...
    for (_i = 0, _len = profiles.length; _i < _len; _i++) {
      prs = profiles[_i];
      _results.push(localStorage.removeItem(prs));
    }
    return _results;
  };
  ...
  (function($, global, undefined_) {
    ...
    MB.Core.switchModal = function(options) {
      var contNew, content, form, table;
      ...
      if (options.type) {
        if (options.type === "form" && options.name) {
          ...
          if (options.isNewContent) {
            contNew = new MB.ContentNew(options);
            contNew.create();
          } else {
            if (MB.Modal.opened) {
